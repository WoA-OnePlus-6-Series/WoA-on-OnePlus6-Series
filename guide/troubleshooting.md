...
```cmd
adb push path\to\fsc.bin /tmp/ & adb shell dd if=/tmp/fsc.bin of=/dev/block/by-name/fsc
```

```cmd
adb push path\to\fsg.bin /tmp/ & adb shell dd if=/tmp/fsg.bin of=/dev/block/by-name/fsg
```

```cmd
adb push path\to\modemst1.bin /tmp/ & adb shell dd if=/tmp/modemst1.bin of=/dev/block/by-name/modemst1
```

```cmd
adb push path\to\modemst2.bin /tmp/ & adb shell dd if=/tmp/modemst2.bin of=/dev/block/by-name/modemst2
```
